...
class Usuari extends Model {
    ...
    public function getEntitat() {
        ...
        $idEntitat = $this->attributes['idEntitat'];
        switch ($tipus) {
            case 10: {
                    $entitat = Professor::where('idProfessor', $idEntitat)->first();
                    break;
                }
            case 20: {
                    $entitat = Empresa::where('idEmpresa', $idEntitat)->first();
                    break;
                }
            case 30: {
                    $entitat = Alumne::where('idAlumne', $idEntitat)->first();
                    break;
                }
            default: {
                    $entitat = null;
